...
test('lab command article prerenders its approved metadata and evidence', () => {
    ...
    assert.match(html, /237 deduplicated records carrying an anomaly class/);
    assert.match(html, /recovery labels are 2\.60× more likely than random instants in the same log to have at least one parseable operation-log row nearby/);
    assert.match(html, /the enrichment remained at 2\.80×/);
    assert.match(html, /the enrichment still remained around 2\.8×/);
    assert.match(html, /<strong>So those boundary events alone do not explain the result\.<\/strong>/);
    assert.match(html, /unanswerable rather than negative/);
    assert.match(html, /temporal association/);
    assert.match(html, /recipe-engine events/);
